...
from utils_deberta import convert_examples_to_features, processors
from modeling_deberta import (
    DebertaV2ForMultipleChoice,
    DebertaV2ForMultipleChoiceSACL1,    # self-attn: A
)
from modeling_deberta2 import (
    DebertaV2ForMultipleChoiceSACL2,    # self-attn: B
)
from modeling_deberta3 import (
    DebertaV2ForMultipleChoiceSACL3,    # Transformer: A
)
from modeling_deberta4 import (
    DebertaV2ForMultipleChoiceSACL4,    # Transformer: 20th-23rd
)
...
MODEL_CLASSES = {
    ...
    # "deberta": (DebertaV2Config, DebertaV2ForMultipleChoiceSACL1, DebertaV2Tokenizer),
    # "deberta": (DebertaV2Config, DebertaV2ForMultipleChoiceSACL2, DebertaV2Tokenizer),
    "deberta": (DebertaV2Config, DebertaV2ForMultipleChoiceSACL3, DebertaV2Tokenizer),
    # "deberta": (DebertaV2Config, DebertaV2ForMultipleChoiceSACL4, DebertaV2Tokenizer),
}
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {
                "input_ids": batch[0],  # (bsz, 4, max_len)
                "attention_mask": batch[1],
                "token_type_ids": batch[2]
                if args.model_type in ["bert", "xlnet", "deberta"]
            ...
            # logits = outputs[1]
            # NEW
            loss, logits = outputs["loss"], outputs["logits"]   # logits_shape = (bsz, 4)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if evaluate:
            ...
            examples = processor.get_test_examples(test_path)
        else:
    ...
    all_input_ids = torch.tensor(select_field(features, "input_ids"), dtype=torch.long) # (total, 4, max_len)
    all_input_mask = torch.tensor(select_field(features, "input_mask"), dtype=torch.long)
    all_segment_ids = torch.tensor(select_field(features, "segment_ids"), dtype=torch.long)
    all_label_ids = torch.tensor([f.label for f in features], dtype=torch.long) # (total,)
...
def main():
    ...
    parser.add_argument("--beta", default=0.5, type=float, help="Beta for CL loss.")
    ...
    if args.server_ip and args.server_port: # XXX
    ...
    else:  # XXX # Initializes the distributed backend which will take care of sychronizing nodes/GPUs
    ...
    if args.local_rank not in [-1, 0]:  # XXX
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        if args.eval_all_checkpoints:   # XXX
            checkpoints = list(
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        checkpoint_dir = os.path.join(args.best_checkpoint)
        model = model_class.from_pretrained(checkpoint_dir)
        model.to(args.device)
        _, preds = evaluate(args, model, tokenizer, test=True)
        np.save(os.path.join(args.output_dir, "best_test_preds.npy" if args.output_dir is not None else "test_preds.npy"), preds)
